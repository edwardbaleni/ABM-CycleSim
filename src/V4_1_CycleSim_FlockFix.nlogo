...
to pack
  ...
  ifelse any? cohesion-group [
      find-nearest-neighbor
      ifelse distance next-neighbour < 0.5;nearest-neighbor < 0.5
        [ separate ]
        [ align
...
to find-alg-group
  set cohesion-group other turtles in-cone 140 20
  set separation-group other turtles in-cone 140 0.5
end
...
to find-nearest-neighbor ;; turtle procedure
  ...
  set next-neighbour min-one-of cohesion-group [distance myself]
end

to separate
  turn-away ([heading] of next-neighbour) sep  ; set angle that cyclicst can turn away from
   ;ifelse random-float 1 < 0.75 [ turn-sep] [slow-down]       ; If I want my agents to either slow down or turn away
end

to turn-sep
  turn-away ( [heading] of next-neighbour) sep
end
