...
to acceleration
  set accel-cx 0.01 * ( find-average-x - xcor )
  set accel-cy 0.01 * ( find-average-y - ycor )
  ...
  (foreach x-of-sep y-of-sep [
   [x y] ->
    set asx sqrt ( ( xcor - x ) ^ 2 + ( ycor - y ) ^ 2 )

    if asx > 1 [
      set accel-sx accel-sx + ( (xcor - x) / asx );( abs (xcor - x ) ) ^ 2 )
      set accel-sy accel-sy + ( (ycor - y) / asx );( abs (ycor - y ) ) ^ 2 )
    ]
  ])

  set accel-sx 0.1 * accel-sx
  set accel-sy 0.1 * accel-sy

  set accel-x accel-cx + accel-sx
  set accel-y accel-cy + accel-sy
...
to-report x-of-sep
  report [ xcor ] of separation-group
end

to-report y-of-sep
  report [ ycor ] of separation-group
end

to-report find-average-x
  report mean [xcor] of cohesion-group
end

to-report find-average-y
  report mean [ycor] of cohesion-group
end

to separate
  ;turn-away ([heading] of nearest-neighbor) sep  ; set angle that cyclicst can turn away from
   ifelse random-float 1 < 0.75 [ turn-sep] [slow-down]       ; If I want my agents to either slow down or turn away
end
